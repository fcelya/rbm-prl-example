...
class CFRBM:
    def __init__(
        ...
        initial_v_x=None,
        initial_v_g=None,
        initial_weigths_x=None,
        initial_weights_g=None,
        debug=False,
    ):
        ...
        if initial_weigths_x:
            ...
            initial_weights_x = np.array(
                np.random.normal(0, 0.1, size=self.dim_x), dtype=np.float32
            )
            initial_weights_g = np.array(
                np.random.normal(0, 0.1, size=self.dim_g), dtype=np.float32
            )
        ...
        self.weights_x = theano.shared(
            value=initial_weights_x, borrow=True, name="weights_x"
        )
        self.weights_g = theano.shared(
            value=initial_weights_g, borrow=True, name="weights_g"
        )
    ...
    def prop_up(self, vis, gen):
        return tt.nnet.sigmoid(
            tt.dot(vis, self.weights_x) + tt.dot(gen, self.weights_g) + self.hbias
        )

    def sample_hidden(self, vis, gen):
        activations = self.prop_up(vis, gen)
        h1_sample = self.theano_rng.binomial(
            size=activations.shape, n=1, p=activations, dtype=theano.config.floatX
        )
    ...
    def sample_visible_x(self, h, k=5):
        ...
        v1_sample_x = self.theano_rng.binomial(
            size=activations.shape, n=1, p=activations, dtype=theano.config.floatX
        )
    ...
    def sample_visible_g(self, h, k=1):
        ...
        v1_sample_g = self.theano_rng.binomial(
            size=activations.shape, n=1, p=activations, dtype=theano.config.floatX
        )
    ...
    def gradient(self, v1_x, v1_g, h1, v2_x, v2_g, h2p, masks_x, masks_g):
        ...
        gw_x = (
            (outer(v1_x, h1) * v1_xh1_mask_x) - (outer(v2_x, h2p) * v1_xh1_mask_x)
        ).mean(axis=0)
        gw_g = (
            (outer(v1_g, h1) * v1_gh1_mask_g) - (outer(v2_g, h2p) * v1_gh1_mask_g)
        ).mean(axis=0)
    ...
    def cdk_fun(
        ...
        (
            ...
            h2,
            h2a,
        ) = self.contrastive_divergence_1(vis_x, vis_g)
        ...
        for i in range(k - 1):
            (
                ...
                h2,
                h2a,
            ) = self.contrastive_divergence_1(v2_x, v2_g)

        (W_x, W_g, V_x, V_g, H) = self.gradient(
            v1_x, v1_g, h1, v2_x, v2_g, h2a, masks_x, masks_g
        )
        ...
        updates = [
            (
                self.weights_x,
                cast32(self.weights_x + (momentum * self.prev_gw_x) + (W_x * w_lr)),
            ),
            (
                self.weights_g,
                cast32(self.weights_g + (momentum * self.prev_gw_g) + (W_g * w_lr)),
            ),
            (
                self.vbias_x,
                cast32(self.vbias_x + (momentum * self.prev_gv_x) + (V_x * v_lr)),
            ),
            (
                self.vbias_g,
                cast32(self.vbias_g + (momentum * self.prev_gv_g) + (V_g * v_lr)),
            ),
    ...
    for j in range(epochs):
        ...
        for batch_i, batch in enumerate(chunker(list(profiles.keys()), batch_size)):
            ...
            for movieid in batch:
                ...
                genre_profile = [0.0] * len(all_genres)
        ...
        for batch in chunker(list(tests.keys()), batch_size):
            ...
            movie_predictions = revert_expected_value(
                predict(test_batch_x, test_batch_g)
            )
        ...
        prec_rec = precision_recall_fscore_support(
            true_rat < 3, pred_rat < 3, average="binary"
        )
        print(prec_rec)
...
if __name__ == "__main__":
    ...
    for experiment in experiments:
        ...
        all_users, all_movies, all_genres, tests = load_dataset(
            train_path, test_path, item_path, sep, user_based=False
        )
